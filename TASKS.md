...
- **状态**：`[x]` done — 2026-07-03 / commit pending
- **注**：`findVillageInChunk` 已补全——用 `getChunkSource().getChunkNow()`（不 force-load）+ `ChunkAccess.getAllStarts()` + Structure registry 过滤 5 种 village，返回 `BoundingBox.getCenter()`。
